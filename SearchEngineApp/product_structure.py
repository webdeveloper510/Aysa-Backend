...
def AllProductDetailMain(file_path, embedding_dir_path ,ModelDirPath , TransferModelDir):
    try:
...
def BrandProductMain(file_path, embedding_dir_path ,ModelDirPath , TransferModelDir):
    try:
        product_structure = BrandModelPredictorSemantic(file_path)
        model = product_structure.DownloadUpdateModel(TransferModelDir)
        df = product_structure.read_csv()
        cleaned_df = product_structure.preprocess_text_data(df)
        encoded_df, embeddings = product_structure.encode_text(cleaned_df)
